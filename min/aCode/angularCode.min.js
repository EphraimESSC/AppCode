...
/* Min file (c) */
!function(){"use strict";function a(){return{restrict:"A",link:function(a,b,c,d){"controlgroup"===c.role&&$(b).controlgroup({corners:!1})}}}angular.module("ui").directive("role",a),a.$inject=[]}();
...
/* Min file (c) */
!function(){function a(){return function(a,b,c){return _.filter(a,function(a){return a[b]===c})}}function b(a,b,c,d){function e(){i.values={},i.values.tabInfo=[{properties:{id:"rootId"},type:"TabRoot",children:[]}],f(),g()}function f(){b.getValue("diagnoseTabModel").then(function(b){if(b){b.tabInfo&&a.scanAndUpdateDuplicateIds(b.tabInfo),i.values=b;var d=a.scanAndUpdateDuplicateIds(angular.copy(i.values.tabInfo));c.setControls(d)}})}function g(){a.fetchServiceValues().then(function(a){i.controlsServiceValues=a,angular.forEach(i.values.tabInfo[0].children,function(b,c){if(b.selectedList){var d=_.findIndex(a.lists,function(a){return a.listName===b.selectedList.listName});-1!==d&&(b.selectedList=a.lists[d])}b.categoriesList&&(d=_.findIndex(a.lists,function(a){return a.listName===b.categoriesList.listName}),-1!==d&&(b.categoriesList=a.lists[d]))}),setTimeout(function(){$("#defaultPage").enhanceWithin(),$("#diagnoseConfigSection select.selected-list").selectmenu("refresh")},1)})}var h=this,i=h.model={};h.initialize=e,e(),h.onChangeTabChoice=function(){},h.onFileInput=function(b){console.log("file",b.files);var c=$(b).attr("index"),d=new FileReader;d.onload=function(d){i.values.tabInfo[0].children[c].fileName=b.files[0].name,i.values.tabInfo[0].children[c].children&&a.removeIdsFromCacheList(i.values.tabInfo[0].children[c].children);var e=JSON.parse(d.target.result);a.scanAndUpdateDuplicateIds(e),i.values.tabInfo[0].children[c].children=e},d.readAsText(b.files[0])},h.onChangeList=function(a){i.values.tabInfo[0].children[a].children=i.values.tabInfo[0].children[a].selectedList.controls},h.onChangeCategories=function(a){var b=i.values.tabInfo[0].children[a].categoriesList.controls;i.values.tabInfo[0].children[a].categories=b;var c=[];_.deepMapValues(b,function(a,b){"label"===b[b.length-1]&&c.push(a)}),console.log(c),i.values.tabInfo[0].children[a].categoriesListArray=c},h.onDeleteDiagnoseTab=function(b){a.removeIdsFromCacheList(i.values.tabInfo[0].children[b].children),a.removeIdsFromCacheList(i.values.tabInfo[0].children[b].categories,"categorieschildren"),i.values.tabInfo[0].children.splice(b,1)},h.onAddDiagnoseTab=function(){i.values.tabInfo[0].children.push({tabName:"Diagnose Tab "+(i.values.tabInfo[0].children.length+1),type:"Tabs"}),setTimeout(function(){$("#defaultPage").enhanceWithin()},1)},h.onSaveDiagnoseConfig=function(){b.setValue("diagnoseTabModel",i.values);var d=a.scanAndUpdateDuplicateIds(angular.copy(i.values.tabInfo));c.setControls(d),ConfigTabFunctionality.setAppRefreshPending()}}angular.module("myapp.diagnose-config",["controls-module","db-service","data"]).filter("property",a).controller("DiagnoseConfigController",b),b.$inject=["ControlsService","DBService","SharedDataService","$scope"]}();
